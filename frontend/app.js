...
document.getElementById("connectWalletBtn").onclick = async () => {
  try {
    ...
    // Check if the wallet has been deleted
    const walletHistory = JSON.parse(
      localStorage.getItem("walletHistory") || "[]"
    );
    const walletExists = walletHistory.some(
      (entry) => entry.address === account
    );

    if (!walletExists) {
      console.log("New wallet or deleted wallet. Proceeding to connect.");
    }
    ...
    // Set the network name dynamically in the "Connected Network" field
    document.getElementById("networkName").value =
      netName.charAt(0).toUpperCase() + netName.slice(1);

    // Display the network name in the account section as well (optional)
    ...
    if (!walletExists) {
      saveWalletHistory(account); // Only save if it's a new or previously deleted wallet
    }
  } catch (err) {
    console.error(err);
...
document.getElementById("createPlanBtn").onclick = async () => {
  try {
    ...
    // Handle custom interval if selected
    if (interval === "custom") {
      const customMinutes = Number(
        document.getElementById("customInterval").value
      );
      if (isNaN(customMinutes) || customMinutes <= 0)
        return alert("Please enter a valid custom interval (in minutes).");
    ...
    // Dynamically get the network name and use it here
    const network = await provider.getNetwork();
    const networkName = network.name.toLowerCase(); // Example: 'sepolia', 'mainnet', etc.
    ...
    // Create the EMI plan on the smart contract
    const tx = await contract.createEmiPlan(
       // The sender is not needed here, as it's the receiver's plan
      emiAmount,
      interval,
      totalAmount,
      networkName
    ...
    // Generate the QR code for the sender to scan
    const receiverAddress = await signer.getAddress();
    const qrData = JSON.stringify({ receiver: receiverAddress, planId });
    ...
    // Handle errors during the EMI plan creation
    console.error(err);
...
function saveWalletHistory(address) {
  ...
  // Get the existing wallet history from localStorage
  let list = JSON.parse(localStorage.getItem("walletHistory") || "[]");
...
//when gas error came, that code is this
// let provider;
// let signer;
// let contract;
// let planId = 1;
...
//     // Display the network name in the account section as well (optional)
//     document.getElementById("network").innerText =
//       "Network: " + netName.charAt(0).toUpperCase() + netName.slice(1);
